      ...
      measure.chords.foreach {
        chord => {
          chord.notes.foreach { note =>
            val noteOnTime = chord.date
            val noteOffTime = noteOnTime + note.dur

            context.system.scheduler.scheduleOnce((noteOnTime) milliseconds)(note_on(note.pitch, note.vol, 10))
            context.system.scheduler.scheduleOnce((noteOffTime) milliseconds)(note_off(note.pitch, 10))
          }
        }
      }
    }
